    ...
    *,
    ...
    def _run() -> str | None:
        ...
        command = [
            ...
            "--output-dir",
            str(output_dir),
            # Warm-start from the current MLflow @champion. A triggered retrain
            # has no local checkpoint to pass, and run_local_pipeline requires a
            # pretrain source, so this is the on-prem/automated equivalent of
        ...
        # lands in the cron container logs.
        result = subprocess.run(command, cwd=str(REPO_ROOT), check=False)
        if result.returncode != 0:
            # Pipeline failed (e.g. gate not passed, missing pretrain source,
            # training error). Treat as "no candidate registered" rather than
            # crashing the checker, so the cron tick reports a clean decision
            # and the next tick can retry.
            logger.error(
                "Local pipeline exited with code %s — no candidate registered.",
                result.returncode,
            )
            return None
